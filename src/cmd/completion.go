...
Bash:
  $ source <(vaultreader completion bash)
  # To persist:
  $ vaultreader completion bash | sudo tee /etc/bash_completion.d/vaultreader > /dev/null

Zsh:
  $ vaultreader completion zsh > ~/.zsh/_vaultreader
  $ echo 'fpath=($HOME/.zsh $fpath)' >> ~/.zshrc
  $ echo 'autoload -Uz compinit && compinit' >> ~/.zshrc
  # Or, for current session:
  $ source <(vaultreader completion zsh)
`,
}
